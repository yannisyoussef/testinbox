...
| 3 | The **Postfix relay hop is part of the automated rehearsal**, proving end to end: active recipient accepted; nonexistent recipient body never persisted; multi-recipient behaviour; downstream `451`/retry semantics; message-size limits; connection/recipient abuse limits; successful delivery. Discharged by TI-005 — see [`docs/architecture/mail-edge-contract.md`](../architecture/mail-edge-contract.md) for what each proof asserts and what remains uncovered. | Application |
