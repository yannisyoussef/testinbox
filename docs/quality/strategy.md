...
| Credential lifecycle | Domain + application + Postgres integration + API + e2e + product synthetic | ADR-032: format parsing (truncation, transposition, an unknown future version), constant-time verification, revocation taking effect on the next request with no cache to wait out, expiry, scope enforcement and the refusal to grant a scope the creator lacks, the bootstrap window closing and reopening, coalesced `last_used_at` writes across nodes, and the negative proofs — no representation but the single `201` carries a credential, and nothing reaches the logs at debug level. |
