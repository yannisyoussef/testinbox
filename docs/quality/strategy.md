...
| Concurrency | Targeted tests | Concurrent `createInbox()` token collisions, concurrent waiters on one inbox, wait-request cancellation/resource cleanup, inbox-expiry-vs-inbound-delivery race (`docs/architecture/inbound-mail-flow.md`). |
