...
| Endpoint | MVP? | Notes |
|---|---|---|
| `POST /v1/inboxes` | Yes | Body: `ttl`, optional `aliasHint` (not a guaranteed exact address — ADR-008). Supports `Idempotency-Key`. |
...
| `POST /v1/inboxes/{id}/messages/wait` | Yes | The core primitive — see [`wait-semantics.md`](../architecture/wait-semantics.md). Long-poll, capped duration, `408` on timeout. |
