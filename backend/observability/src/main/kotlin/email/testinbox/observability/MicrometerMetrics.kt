...
class MicrometerWaitMetrics(
    ...
    init {
        // A gauge that only ever climbed would make a handle leak look like
        // load, which is the regression this exists to catch.
        registry.gauge("testinbox_wait_requests_active", active) { it.get().toDouble() }
    }
...
class MicrometerNotifierMetrics(
    ...
    init {
        ...
        registry.gauge("testinbox_wait_listen_degraded_polling", degradedFlag) { it.get().toDouble() }
    ...
    override fun operationCompleted(
        ...
        registry
            .timer(
                ...
                Tags.of("operation", operation.name, "outcome", if (success) "success" else "failure"),
            ).record(duration)
    }
}
...
class BuildInfoMetric(
    ...
    init {
        ...
        registry.gauge(
            "testinbox_build",
            Tags.of("service", service, "git_sha", gitSha, "version", version),
            1.0,
        ) { it }
    }
}
